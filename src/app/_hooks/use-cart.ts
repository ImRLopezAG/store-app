...
export function useCart() {
  ...
  function calculateTotals() {
    ...
    const totals = data.lines.reduce(
      (acc, product) => {
        const subtotal = round2Decimals(
          acc.subtotal + getProductTotalExcludingVAT(product)
        )
        const totalVAT = round2Decimals(
          acc.totalVAT + getProductVAT(product) * product.quantity
        )
